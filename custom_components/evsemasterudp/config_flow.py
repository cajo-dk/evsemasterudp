...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if host:
            ...
            _LOGGER.info(
                "Using configured EVSE endpoint %s @ %s:%s in direct mode",
                serial,
                host,
                port,
            )
        ...
        if not success:
            ...
            diagnostics = client.get_login_diagnostics(serial)
            if diagnostics.get("auth_failure_reason") == "incorrect_password":
                raise InvalidAuth
            if diagnostics.get("auth_failure_reason") == "no_login_response":
                raise CannotConnect
            raise InvalidAuth
